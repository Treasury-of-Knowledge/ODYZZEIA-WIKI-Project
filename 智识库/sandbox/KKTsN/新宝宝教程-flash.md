...
这样会直接更新到主分支上的最新版（截止20251124是1.14.0）,更新到某个指定版本的方法可问AI或加入Discord社区后自行搜索
...
3. Google 模型 选择`gemini 2.5 pro` **（没有任何preview exp 日期等后缀）**

> 如果这一步在Google 模型列表中找不到gemini-2.5-pro，说明使用的酒馆版本太低，请更新到最新版
{.is-info}
...
- 基础限额为: 每个项目每天50条，3百万tokens (可以简单理解成字数)，TPM为12.5万 (可理解为最大上下文长度)
- 每个账号可以拥有多个项目，项目之间配额独立
- 也就是说 在一个号上开多个项目即可让单号限额翻好几倍，具体教程可在类脑搜索

再额外：谷歌的其他模型

- Gemini 3.0 Pro 截至20251124还在测试，未正式在AIS API渠道上线，想玩可以来类脑搜教程
- Gemini 2.5 Flash 是更快但能力更弱的模型；Flash Lite是最快，能力也最弱的模型，玩酒馆建议用Pro
...
2. 若提示超出的限额为'TokensPerDay'或'RequestsPerDay':
  ...
  - 这个key超过了当天的限额（2.5pro为每天50条/3百万token)
  - 换一个来自不同项目的key用
