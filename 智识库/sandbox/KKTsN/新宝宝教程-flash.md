...
# 0 简介
   ...
   1. 绑卡薅GCP赠金玩Vertex/试用Vertex等
   2. 写卡，写世界书，写预设等
   3. 云酒馆

   > 所有'xxx'都代表'这里应该有内容，但不一定是什么内容'，**不是真的几个x**
...
---
   ...
   **作者的环境: Windows 10 22H2**
   ...
   > 为git配置代理是确保git clone（即安装酒馆）时能享受到梯子的效果
...
---
   ...
   1. 打开你的代理工具，找到它使用的端口（例如clash verge默认是7897, 而v2ray默认是10808）
      如果不知道在哪里找，可以在网上搜索'xxx（代理工具的名字）怎么查看使用的端口'
   2. 在PowerShell中输入:
...
一键脚本教程在类脑智识库的宝宝教程-部署教程里也有，即最下方的安卓部署教程
   ...
   > **1.13.4以下版本的酒馆建议更新到1.13.4及以上，但是1.13.5这个版本的bug极多，基本无法正常使用，不要更新到这个版本**
   > {.is-warning}
   ...
   这样会直接更新到主分支上的最新版（截止20251207是1.14.0）,更新到某个指定版本的方法可问AI或加入Discord社区后自行搜索
   ...
   不过你也可以按后文`备份与恢复数据`那一块，用压缩包重新装一个新版的，再把旧版数据手动覆盖回去

   ### 2.1.3 安卓一键脚本

   安卓一键脚本一般都有更新酒馆的功能，**确保网络环境稳定**后在一键脚本中选择更新酒馆即可

   也可以用git pull的方法，具体教程可以问AI或者加入DC社区后自行搜索
...
## 注意，不是所有预设都适合用‘严格’这个后处理选项的，具体看你预设作者的要求

   ![iuuiuevi3.png](/all_upload_files_should_in_here/sandbox_area/kktsn/iuuiuevi3.png)

   4. 然后点最左边的按钮，这就是预设界面，看一眼就行。注意酒馆自带的这个Default预设有很多问题，不要用它，获取预设教程在后面
   ...
   新版（1.14.0）酒馆在这里又加了不少按钮，不过都不影响，不需要特地去动它们
   ...
   > - 优点: 中国大陆可直连，可流式输出（就像在网页/手机用ds一样，随着回复的生成，逐词逐句地显示），DeepSeek 3.2的官方API对酒馆使用来说很便宜
   > - 缺点: 上下文（即AI一次能处理的文字量）较短，模型能力一般
   > {.is-info}


   这里仅介绍官方渠道，硅基/火山/OR等第三方渠道教程请自行搜索（1.14.0酒馆已支持硅基）

   第1步: 打开[DeepSeek开放平台](https://platform.deepseek.com) 注册登录
   ...
   第3步: 点左侧的`用量信息`或`充值`，充一点钱（支持微信支付宝等国内支付渠道）
   ...
   DeepSeek已于9月29日发布了V3.2模型，同样酒馆中的chat对应无思维链，reasoner对应有思维链。具体用哪个请参考你用的预设的作者的要求

   ### 4.1.2 Gemini

   > - 优点: ~~完全免费+额度高，常用模型2.5Pro的能力还可以，几乎无审查~~ 2.5Pro又不给免费额度了，但其他渠道还能玩到
   > - 缺点: 直连对网络（梯子）要求高、涉及敏感内容无法流式输出、有时谷歌服务器爆炸导致用不了
   > {.is-info}

   这里仅介绍使用AI Studio API的渠道，教程和图片都来自类脑社区Moli

   由于AI Studio API渠道不给用2.5pro了，这部分可能没啥价值，来找API用的建议直接去看Cli和Build反代的教程，但是道馆题目还包含这一块的内容。呃反正高级道馆也是5选3，不改了
   [CLI本地 ](https://discord.com/channels/1134557553011998840/1405524233823457300) / [CLI公益站/在线获取凭证](https://discord.com/channels/1134557553011998840/1410747429359325214/1410747429359325214) / [Build ](https://discord.com/channels/1134557553011998840/1380129283430940712) /[另一版Build](https://discord.com/channels/1134557553011998840/1440552338824757280/1440552338824757280) 
   ...
   2. 填写了一堆信息后，谷歌会要求手机号验证（如果梯子质量很好的话可能不需要），可以尝试用+86手机号验证或者使用接码平台

   如果实在无法注册，也可以直接去买，涉及付费这里不作推荐

   第2步: 创建项目和key

   1. 打开[gcp控制台](https://console.cloud.google.com/projectcreate) ，登录谷歌账号
   ...
   3. Google 模型 选择`gemini 2.5 flash` **（没有任何preview exp 日期等后缀）**

   > 如果这一步在Google 模型列表中找不到gemini-2.5-flash，说明使用的酒馆版本太低，请更新到最新版
   > {.is-info}
   ...
   > **用default预设点2.5系列的测试消息可能会导致空回报错，这不是网络或者酒馆的问题 点测试之前先找个预设用**
   > {.is-warning}


   额外: Gemini限额 (1207更新)

   - 基础限额为: flash每个项目每天20条，pro无免费额度
   - 每个账号可以拥有多个项目，项目之间配额独立
   - 也就是说 在一个号上开多个项目即可让单号限额翻好几倍，具体教程可在类脑搜索

   再额外：谷歌的其他模型

   - Gemini 3.0 Pro 截至20251207无免费额度，想玩可以来类脑搜教程
   - 2.5 Pro自20251207开始又没有免费额度了，想白玩来类脑搜各种反代教程
   ...
   1. 退款流
      购买官网会员，抓Cookie反代，并在订阅期结束前把号封掉，可以得到退款，实现白玩
      可以参考智识库的[这个教程](https://wiki.类脑.org/%E6%99%BA%E8%AF%86%E5%BA%93/%E7%B1%BB%E8%84%91%E6%95%99%E7%A8%8B/%E8%8E%B7%E5%8F%96AI%E6%B8%A0%E9%81%93/Pro%E9%80%80%E6%AC%BE%E6%B5%81%E6%95%99%E7%A8%8B)
   2. 各种内置Claude的东西的反代（比如编程工具Cursor，Kiro等）
      教程可以到类脑里面找
   3. 官方API
      非常贵，不推荐
   ...
   如何获取:
   第1步: 打开discord社区的预设区
   [类脑社区预设区](https://discord.com/channels/1134557553011998840/1134822069222264874)
...
## 补充：现在很多预设都用到了'预设绑定正则'等插件，不需要额外下载正则了；也有很多预设将一张角色卡作为'正则卡'，导入这张角色卡并将卡的局部正则转换为全局正则就行
   ...
   如何获取:
   第1步: 在 Discord 类脑的频道列表中找到**角色卡区**，这里被分成了多个频道，点进去就能看到很多角色卡

   ![kaqu.png](/all_upload_files_should_in_here/sandbox_area/kktsn/kaqu.png)

   > 类脑的**帖子备份区** 多数帖子没有卡，**拟态复原区的卡（如果有）在最上面和最下面两层**
   > {.is-info}

   第2步: 卡的作者会指路哪一层的**图片就是卡**，下载这张图片的原图（PNG格式！）就可以导入酒馆了

## 注意，下下来不是png格式就是下错了，什么webp jpg格式都是下错了，不要自己改文件格式没有用的，去重新下正确的卡
   ...
   - 在Discord 类脑的频道列表中找到 工具/插件/美化/世界书 区，点进去就能看到很多资源
...
---
   ...
   - **思维链:** 也叫'COT'，指预设中要求AI在输出正文前，'思考'并输出的一些内容，如'当前处于何种情境？时间？地点？社会关系？角色当前姿势？'
     - 如果角色卡要求'关闭预设思维链'，只需关闭预设这部分的所有条目，具体请参考预设作者的要求
   ...
   - **摘要:** 也叫'小总结'，让AI在每次回复的正文后带上一段对本次回复中情节的摘要，配合正则可以做到距离当前较远的聊天记录只发送摘要（而不发送全部正文），节省上下文
     - 注意小总结只对开了以后新生成的楼层生效
     - 小总结必须和正则一起使用，一起开一起关，否则没有效果

   - **字数控制:** 要求AI每次输出的正文不能超过/低于xx字
     - 由于LLM的原理导致它们并不知道自己写了多少字，并不会遵守得很好

   - **总结前文:** 也叫'大总结'，一般在预设靠近底部的位置。要求AI停止剧情推进或角色扮演，在本次回复中仅进行对前文的总结

   - **尾部:** 预设最靠下的几个条目。这部分用来绕过输入截断，强调用户本次输入、预填充AI输出内容等
   ...
   **每节的标题是后台报错信息中的关键字**
   ...
   2. 若提示超出的限额为'TokensPerDay'或'RequestsPerDay':
     ...
     - 这个key超过了当天的限额（2.5pro已无免费额度，2.5flash每天20条)
     - 换一个来自不同项目的key用

   3. 若报错信息很短，没有明确提示超出哪项限额，而且含有关键词(e.g. check quota):
   ...
   131K导致的500已消失，现在出500先检查网络问题，还不行截图后台到类脑答疑区问
   ...
   6.3.10 Unexpected token 'd' ……

   - d其实是流式块的data，这个报错大概是因为反代或者API那边没有正确处理流式/假流式的bug
   - 试着开关流式传输/用真流式/问问反代作者
...
# 7 DC社区有关常见问题
   ...
   其余DC相关问题请到DC里面答疑区问，或者看看道馆挑战的教程，你想要的里面都有

---

   结束，感谢读到这里

   如果发现本教程有问题或者有已经过时的地方，可以到DC类脑找作者@kk1189_61801
